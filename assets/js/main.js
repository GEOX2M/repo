...
(function () {
	...
	// Translations object (kept intact). Some entries include HTML so we use innerHTML when required.
	const translations = {
		fr: {
			...
			formation02content: "J'ai appri à utiliser ce framework en suivant une formation de 106 heures donnée par Stephen Ulibarri intitulée 'Unreal Engine 5 - Gameplay Ability System - Top Down RPG'</a>",
		...
		en: {
			...
			formation02content: "I learned how to use this framework by taking a 106-hour course given by Stephen Ulibarri entitled ‘Unreal Engine 5 - Gameplay Ability System - Top Down RPG’</a>",
	...
	// Keep old global names used in markup for compatibility (prefer using app.* in new code)
	window.changeLanguage = changeLanguage; // used by inline handlers
	window.download = downloadCV;
	window.redirectTo = redirectTo;

	// UI behaviour that runs on load
	function onLoadInit() {
		// Set initial language
		changeLanguage(currentLang);

		// Show portfolio if present
		const portfolio = document.getElementById('Portfolio') || document.getElementById('portfolio');
		if (portfolio) portfolio.classList.add('show');
	...
	// Toggle portfolio visibility, attached to button if present
	function attachPortfolioToggle() {
		const toggle = document.getElementById('show-portfolio');
		if (!toggle) return;

		toggle.addEventListener('click', () => {
			const p = document.getElementById('Portfolio') || document.getElementById('portfolio');
			if (!p) return;
	...
	// Polyfills / helpers (kept minimal)
	// classList polyfill for very old browsers (kept from upstream)
	(function () {
		if ('classList' in document.documentElement) return;
		// very small polyfill (omitted here for brevity) - in practice keep original long polyfill if you need old-IE support
	}());

	// canUse helper
	window.canUse = window.canUse || function (p) {
	...
	// Slideshow Background (kept behaviour but cleaned up)
	(function () {
		const settings = {
			images: {
		...
		const bgs = Object.keys(settings.images).map((k) => {
			const el = document.createElement('div');
			el.style.backgroundImage = `url("${k}")`;
			el.style.backgroundPosition = settings.images[k];
